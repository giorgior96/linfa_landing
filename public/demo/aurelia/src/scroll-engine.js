...
function mountScrollWorld(container, config) {
  ...
  // Phone detection. A narrow desktop panel can be 550–800px wide while still using a
  // precise pointer; treating it as a phone removed the cinematic video entirely.
  // Coarse input always means mobile, while width alone only switches below 520px.
  ...
  const useMaster = Boolean(config.masterClip) && !isMobile() && !reduce;
  const masterDurations = Array.isArray(config.masterSegments) && config.masterSegments.length === NSEG
    ? config.masterSegments
    : SEGMENTS.map(() => 1);
  let masterTotal = 0;
  SEGMENTS.forEach((segment, index) => {
    segment.masterStart = masterTotal;
    masterTotal += Number(masterDurations[index]) || 1;
    segment.masterEnd = masterTotal;
  });
  ...
  if (config.atmosphere !== false) {
    ...
    particles = el('div', 'sw-particles'); sky.appendChild(particles);
  }
  ...
  // Desktop can scrub one pre-joined master instead of swapping six independent
  // decoders at the section seams. One decoder + one timeline removes the small
  // stalls caused by clip hand-offs and keeps every scroll delta in the same clock.
  const master = {
    el: null, img: null, video: null, loading: false, ready: false,
    cur: 0, target: 0,
  };
  if (useMaster) {
    const scene = el('div', 'sw-scene sw-scene--master');
    scene.style.opacity = '1'; scene.style.zIndex = '120';
    const img = el('img', 'sw-scene__still');
    img.alt = ''; img.decoding = 'async'; img.src = SECTIONS[0].still || '';
    scene.appendChild(img); stage.appendChild(scene);
    master.el = scene; master.img = img;
  }
  ...
  SEGMENTS.forEach(s => {
    ...
    scene.appendChild(img); stage.appendChild(scene);
    if (useMaster) scene.hidden = true;
  ...
  function loadClip(s) {
    ...
    if (useMaster || reduce || s.loading || !s.clip || (isMobile() && s.clipM === false)) return;
    s.loading = true;
  ...
  function loadMaster() {
    if (!useMaster || master.loading || !config.masterClip) return;
    master.loading = true;
    fetch(config.masterClip).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        const v = document.createElement('video');
        v.className = 'sw-scene__video';
        v.muted = true; v.playsInline = true; v.preload = 'auto';
        ...
        v.addEventListener('loadedmetadata', () => {
          master.ready = true;
          master.cur = master.target;
          const t = clamp(master.target, 0, 0.999) * (v.duration || 1);
          try { v.currentTime = Math.max(0.001, t); } catch (e) {}
          read();
        });
        v.addEventListener('loadeddata', () => {
          master.el.classList.add('has-clip');
          try { v.pause(); } catch (e) {}
        }, { once: true });
        master.el.appendChild(v); master.video = v;
      }).catch(() => { master.loading = false; });
  }
  ...
  function read() {
    ...
    if (useMaster) {
      ...
      const local = clamp((y - segment.start) / (segment.end - segment.start), 0, 1);
      const mapped = segment.linger ? lingerEase(local, segment.linger) : local;
      const nextMasterTarget = (segment.masterStart + mapped * (segment.masterEnd - segment.masterStart)) / masterTotal;
      // A fast flick or route jump should not replay every stale seek on the way.
      // Small deltas retain the eased scrub; large deltas snap the smoothing clock
      // to the newest requested frame and issue one fresh seek when the decoder frees.
      if (Math.abs(nextMasterTarget - master.cur) > 0.065) master.cur = nextMasterTarget;
      master.target = nextMasterTarget;
      if (!master.ready && master.img) {
        master.img.style.transform = `translateX(${stageX - 2}vw) scale(${(1.03 + master.target * 0.05).toFixed(3)})`;
      }
    }
    ...
    if (near !== activeIndex) {
      ...
      window.dispatchEvent(new CustomEvent('scrollworld:sectionchange', {
        detail: { index: near, id: SECTIONS[near].id }
      }));
    }
  ...
  function raf() {
    if (useMaster && master.ready && master.video) {
      // Advance the smoothing clock even while the decoder is resolving a seek.
      // When it becomes free we seek once to the newest position instead of
      // replaying a queue of stale intermediate targets.
      master.cur += (master.target - master.cur) * 0.32;
      if (!master.video.seeking) {
        const dur = master.video.duration || 1;
        const t = clamp(master.cur, 0, 0.9995) * dur;
        if (Math.abs(master.video.currentTime - t) > 1 / 30) {
          try { master.video.currentTime = t; } catch (e) {}
        }
      }
    }
    const eps = isMobile() ? 0.02 : 0.008;   // coarser seek step on phones = fewer decodes
    for (let i = 0; !useMaster && i < NSEG; i++) {
      const s = SEGMENTS[i];
  ...
  window.addEventListener('scroll', () => {
    ...
    window.clearTimeout(scrubIdle);
    scrubIdle = window.setTimeout(() => container.classList.remove('is-scrubbing'), 140);
...
function injectCSS() {
  ...
  .sw-particles{position:absolute;inset:-6% -2%;}
  ...
  .sw-scene__still{} .sw-scene.has-clip .sw-scene__still{opacity:0;} .sw-scene__video{z-index:1;}
  .sw-copylayer{position:fixed;inset:0;z-index:20;pointer-events:none;}
  .sw-copylayer::before{content:"";position:absolute;inset:0;width:min(58vw,780px);background:linear-gradient(90deg,var(--sw-bg) 0%,color-mix(in srgb,var(--sw-bg) 82%,transparent) 34%,color-mix(in srgb,var(--sw-bg) 40%,transparent) 62%,transparent 100%);}
  .sw-copy{position:absolute;left:clamp(18px,5vw,64px);top:50%;transform:translateY(-50%);width:min(42vw,460px);opacity:0;}
  .sw-root.is-scrubbing .sw-copy,.sw-root.is-scrubbing .sw-scene{will-change:opacity,transform;}
